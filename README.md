...
- 从 XLSX 导入姓名、学号和班级白名单，按班群名称自动匹配并审批多个班群；
...
“入群审核”页面支持导入包含“班级、学号、姓名”表头的 XLSX 新生名单。程序只提取这三列，名单保存在本机 `data/join_roster.json` 并被 Git 忽略；电话、生日等其他列不会写入机器人数据。机器人会把“2026级计科1班”等群名称自动对应到“26计科1”名单，只有申请中的姓名和学号同时属于目标班级才自动同意，其余申请保留给管理员人工审核。
...
“宣传主持”页面用于一次性社团、协会或部门宣传活动。管理员可设置活动日期、目标群、统一主持词，并逐行维护开始时间、结束时间、宣传部门和本环节内容。批量导入框可直接识别 `14:00-14:10 团委办公室`、`14:00 14:10 团委办公室` 或 `14:00 团委办公室`，支持多行文本和一行多条，预览后可以替换或追加到现有流程。启用并保存后，机器人会在各环节开始时间自动发送主持消息；也可在明确确认后手动立即发送某一环节。
